...
router.post("/category", async (req, res) => {
  ...
  try {
    (async () => {
      try {
        const data = await query(
          "SELECT * FROM category_list WHERE category_name=?",
          [category_name]
        );
...
router.put("/category/:id", async (req, res) => {
  ...
  try {
    (async () => {
      const q =
        "UPDATE category_list SET `category_id` = ?, `category_name` = ? WHERE category_id = ?";
      const value = [req.body.category_id, req.body.category_name];
