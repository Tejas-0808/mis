  ...
  try {
    (async () => {
      const data = await query(
        "SELECT * FROM master_scheme WHERE mastersch_id = ?",
        masterschemeId
      );
    ...
    (async () => {
      ...
      if (!userExists) {
        (async () => {
          try {
            const data = await query(
              "INSERT INTO master_scheme VALUES(?,?,?,?)",
              [mastersch_id, master_scheme, from_year, to_year]
            );
            console.log(data[0]);
...
router.put("/masterscheme/:id",verifyToken, async (req, res) => {
  ...
  try {
    (async () => {
      const q =
        "UPDATE master_scheme SET `mastersch_id` = ?, `master_scheme` = ?, `from_year` = ?, `to_year` = ?  WHERE mastersch_id = ?";
      const value = [
